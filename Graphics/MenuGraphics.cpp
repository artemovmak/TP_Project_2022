...
MenuGraphics::MenuGraphics() : is_unit_tapped(false),
  ...
  unit_icon_.setPosition(UnitIconPosition);
  building_icon_.setPosition(BuildingIconPosition);
...
bool MenuGraphics::UnitIconContains(Vector<int> v) const {
  return GetUnitIcon().getGlobalBounds().contains(static_cast<float>(v.x),
                                                  static_cast<float>(v.y));
}

bool MenuGraphics::BuildingIconContains(Vector<int> v) const {
